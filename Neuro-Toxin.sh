#!/bin/bash
# Neuro Toxin Client - Advanced Auto-Installer (Linux/Mac)
...
INSTALL_DIR="$HOME/.local/share/Neuro-Toxin"
ERROR_COUNT=0
...
print_header() {
    echo ""
    echo -e "${CYAN}========================================${NC}"
    echo -e "${CYAN}   NEURO TOXIN CLIENT AUTO-INSTALLER${NC}"
...
echo "  - Launch Neuro Toxin client"
...
if [ "$JAVA_MAJOR" = "1" ]; then
    ...
    echo ""
    echo "WHY: The Neuro Toxin launcher requires Java 11+ features"
    echo ""
    read -p "Press Enter to exit..."
    exit 1
fi
...
if [ -d "$INSTALL_DIR" ]; then
    print_info "Directory exists - cleaning old files..."
    # Remove old launcher.jar if it exists
    if [ -f "$INSTALL_DIR/launcher.jar" ]; then
        rm -f "$INSTALL_DIR/launcher.jar"
        print_success "Removed old launcher.jar"
    fi
    # Remove any other old files (optional - uncomment if you want to clean everything)
    # rm -f "$INSTALL_DIR"/* 2>/dev/null
else
...
# ============================================================================
print_step 4 5 "Downloading/Updating Launcher"

# Always download fresh launcher.jar (overwrite if exists)
if [ -f "launcher.jar" ]; then
    print_info "Existing launcher.jar found - will be replaced"
    rm -f launcher.jar
fi
...
print_success "Launcher verified ($FINAL_SIZE bytes) - overwritten if existed"
echo ""

# ============================================================================
# STEP 5: Launch Neuro Toxin
# ============================================================================
print_step 5 5 "Starting Neuro Toxin Client"
